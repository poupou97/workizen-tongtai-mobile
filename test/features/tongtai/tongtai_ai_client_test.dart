...
http.Response _okCompletion(String text, {String? model}) => http.Response.bytes(
...
void main() {
  TongtaiAiClient clientWith(http.Client mock,
  ...
  group('chat — happy path', () {
    test('posts to the xAI endpoint with bearer auth and correct body',
        () async {
      ...
      expect(captured.headers['Authorization'], 'Bearer xai-SECRET-KEY-1234567890');
    ...
    test('omits system message, temperature and max_tokens when not given',
        () async {
      late http.Request captured;
      final mock = MockClient((req) async {
        captured = req;
      ...
      await clientWith(mock).chat(messages: const [TongtaiAiMessage.user('hi')]);
      final body = jsonDecode(captured.body) as Map<String, dynamic>;
  ...
  group('error mapping', () {
    Future<TongtaiAiException> caughtFrom(http.Client mock,
        {Future<String?> Function()? key}) async {
      try {
        await clientWith(mock, key: key)
    ...
    test('401 → invalidKey', () async {
      final e = await caughtFrom(MockClient((_) async => http.Response('', 401)));
      expect(e.kind, TongtaiAiErrorKind.invalidKey);
    });

    test('403 → invalidKey', () async {
      final e = await caughtFrom(MockClient((_) async => http.Response('', 403)));
      expect(e.kind, TongtaiAiErrorKind.invalidKey);
    });

    test('xAI 400 with a bad-key body → invalidKey', () async {
      final e = await caughtFrom(MockClient((_) async => _errBody(400, {
    ...
    test('400 without a key signal → badResponse', () async {
      final e = await caughtFrom(MockClient((_) async => _errBody(400, {
            'error': 'some other validation problem',
          })));
      expect(e.kind, TongtaiAiErrorKind.badResponse);
    });

    test('429 → rateLimit', () async {
      final e = await caughtFrom(MockClient((_) async => http.Response('', 429)));
      expect(e.kind, TongtaiAiErrorKind.rateLimit);
    });

    test('402 → quota', () async {
      final e = await caughtFrom(MockClient((_) async => http.Response('', 402)));
      expect(e.kind, TongtaiAiErrorKind.quota);
    });

    test('500 → serverError', () async {
      final e = await caughtFrom(MockClient((_) async => http.Response('', 500)));
      expect(e.kind, TongtaiAiErrorKind.serverError);
    });

    test('unexpected status → unknown', () async {
      final e = await caughtFrom(MockClient((_) async => http.Response('', 418)));
    ...
    test('TimeoutException → timeout', () async {
      final e = await caughtFrom(MockClient(
    ...
    test('never echoes the provider body into the message', () async {
      final e = await caughtFrom(MockClient((_) async =>
