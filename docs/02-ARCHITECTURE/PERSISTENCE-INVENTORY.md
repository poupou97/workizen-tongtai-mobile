...
| **BusinessContext** (Aggregate Root) | `BusinessContextService` over Orders+Consumer+Inventory repos | Phase 1: metrics + CustomerSummary + OrderSummary + InventorySummary | — (read-only aggregate `BusinessContext`) | ✅ | ✅ | ⛔ | — | ADR-TON-012: **AI reads ONLY this** (never repos). Progressive Aggregation — add opportunity/journey/timeline/goals/finance later. Home consumes it (WTM-129) |
