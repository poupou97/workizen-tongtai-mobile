...
| **BusinessContext** (Aggregate Root) | `BusinessContextService` **composes one Context Provider per capability** (Customer/Order/Inventory/Opportunity/**Journey**/**Finance**) + `BusinessMetricsService` | metrics + CustomerSummary + OrderSummary + InventorySummary + OpportunitySummary (rule-based signal counts) + **JourneySummary** (goals by `GoalPace`) + **FinanceSummary** (income/expense/profit) + BusinessHealth | — (read-only aggregate `BusinessContext`) | ✅ | ✅ | ⛔ | — | ADR-TON-012: **AI reads ONLY this** (never repos). `CapabilityContextProvider<T>` seam — Phase 2 (WTM-133) folded in Journey + Finance; Timeline (derived projection) = one more provider later. Home consumes it (WTM-129/131/133) |
