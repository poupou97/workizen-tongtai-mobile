...
| **BusinessContext** (Aggregate Root) | `BusinessContextService` **composes one Context Provider per capability** (Customer/Order/Inventory/Opportunity/**Journey**/**Finance**/**Timeline**) + `BusinessMetricsService` | metrics + CustomerSummary + OrderSummary + InventorySummary + OpportunitySummary + **JourneySummary** (goals by `GoalPace`) + **FinanceSummary** (income/expense/profit) + **TimelineSummary** (activity-stream projection; excluded from `hasData`) + BusinessHealth | — (read-only aggregate `BusinessContext`) | ✅ | ✅ | ⛔ | — | ADR-TON-012: **AI reads ONLY this** (never repos). `CapabilityContextProvider<T>` seam. Phase 2 (WTM-133) Journey+Finance; Phase 3 (WTM-134) Timeline projection over live repos → **non-AI snapshot complete**. Home consumes it (WTM-129/131/133/134) |
