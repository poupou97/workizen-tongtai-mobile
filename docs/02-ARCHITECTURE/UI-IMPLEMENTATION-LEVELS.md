...
|---|---|---|---|
| `tongtai_home_screen` | BusinessContext + 5 nguồn, một `_HomeData` **+ `businessBriefProvider`** **+ `revenueCapabilityProvider` + `FinanceService`** | `ScreenDataController` + `runTongtaiAction` (seed) | KPI = 0 do đọc hỏng nay là **failed**, không phải "doanh thu 0 ₫". **WTM-304:** thẻ brief nằm ngay dưới hero, **trên** mọi ô số — nó tự lo trạng thái riêng vì đọc theo nhịp khác (Rule Twin trên toàn sổ sách, không phải 5 con số của Home). **WTM-404 (concept-1):** brand header (linh vật · *Tổng Tài AI* · huy hiệu *AI Business OS*) thay `AppBar` chữ "Bảng điều khiển"; hai **hàng thẻ cuộn ngang** (`TtMetricCard`) thay lưới ô đếm + hàng KPI phẳng; "Việc Tổng Tài đề xuất" (chip ưu tiên + nút *Xử lý ngay*) đứng **trên** khối sức khoẻ. Đường xu hướng đọc **Capability Context** doanh thu (ADR-TON-016) + `FinanceSummary.monthly` — Home **không tự cộng chuỗi**. ⛔ Thẻ năng lực **không** có đường: không repository nào giữ ảnh chụp "số sản phẩm hôm qua", nội suy sẽ là xu hướng bịa (WTM-384) |
...
| `tongtai_weekly_review_screen` | WeeklyReviewRule (Rule Twin) | `TongtaiAsyncScreenData` | `insufficient` ≠ **tuần rồi bán 0** ≠ `failed` |
...
| `tongtai_import_screen` | `bundledDemoSourceProvider` / file người bán chọn → `XlsxCommerceSource` → `CommerceImporter` (repository production) + `importJobsProvider` | `runTongtaiAction` ×4 | **WTM-326 · C2 · Epic WTM-324.** Xem trước là **một bước riêng**, không phải hộp thoại xác nhận — gộp lại sẽ biến việc xem xét thành việc bấm OK. Có lỗi **không** chặn cả file: 100 sản phẩm không đáng bị từ chối vì một đơn hỏng; nút chỉ tắt khi không còn gì hợp lệ. Ngôn ngữ nghiệp vụ (*"100 sản phẩm đã sẵn sàng"*), số dòng/tên bảng hiện **nhỏ** bên dưới cho ai muốn mở Excel ra sửa |
| `tongtai_connections_screen` | `connectorCatalogProvider` (catalog + credential store) + `driveBackupListProvider` (Drive) | `TongtaiAsyncScreenData` + `runTongtaiAction` ×3 | **WTM-317 · C1 · Epic WTM-315.** Trạng thái hiển thị **hợp nhất** bản ghi và credential thật: có dòng `ACTIVE` mà mất khoá ⇒ nói `Chưa thiết lập`, vì đó là sự thật. Danh sách bản sao lưu trên Drive hỏng **không** che nút *Sao lưu ngay* — không đọc được bản cũ ≠ không tạo được bản mới. Nút *Ngắt kết nối* chỉ hiện khi có khoá thật để xoá. **WTM-318:** thêm Telegram — ba bước (token · nơi nhận · gửi thử) hiện **cùng lúc**, không phải wizard ba màn; nút *Gửi tin thử* chỉ hiện khi đã `ACTIVE`. **WTM-319:** thêm Jira & Confluence — `validate` gọi API **thật lúc dán khoá**; chưa chọn dự án ⇒ `workContext` là `null` và màn nói *chưa đủ dữ liệu*, **không** nói "0 việc đang mở" |
...
| `tongtai_startup_screen` | đọc `StartupPipeline` (mở CSDL · đếm danh mục/đơn) | pipeline tự nuốt lỗi từng bước, không `try/catch` trong `ui/` | **WTM-367**. Tiến trình là **bước thật đã xong / tổng bước thật** — concept vẽ 68% cố định, không chép. Khởi động nhanh hơn 400ms thì màn này **không được dựng** |
| `tongtai_onboarding_v2_screen` | đọc qua `AnalysisPipeline` → `FirstInsightEngine`; ghi `BusinessProfile` + `BusinessGoal` khi kết thúc | `runTongtaiAction` + `showTongtaiFailure` | **Epic WTM-349**, thay `tongtai_onboarding_conversation_screen` (WTM-178). Bảy chặng trong MỘT `Scaffold`, ba đường (có dữ liệu · dữ liệu mẫu · chưa có). **Chạy trọn không cần AI, không mạng, không tài khoản.** Đường "chưa có dữ liệu" **không đi qua** bước phân tích — ép bằng `stagesFor`, không bằng `if` trong màn. Ghi lỗi **không chặn** người dùng mới vào app |
...
| L3 | WTM-326 (Nhập dữ liệu cửa hàng) · WTM-317 (Kết nối · Google Drive) · WTM-24 (Producer) · WTM-26 (Consumer) · WTM-25/68 (Inventory) · WTM-75 (Customer list) · WTM-87/89 (Journey) · WTM-27/113 (Finance) · WTM-99/100 (Export) · WTM-80/84 (Chat) · WTM-73 (Search) · WTM-63/64/65 (Supplier) · WTM-114 (Timeline) · WTM-160/161 (Forecast/Risk) · WTM-377 (Tổng kết tuần) · WTM-61/83 (AI key BYOK) |
