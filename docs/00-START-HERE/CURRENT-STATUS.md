...
- **Phase 2 (build):** 🔄 IN PROGRESS — developed autonomously by the
  Evidence-Driven Runtime. `flutter analyze` clean; **923/923 tests passing**.
...
| AI | WTM-61 xAI Grok BYOK client + key screen · **WTM-116 G-3A AI Business Summary (ADR-TON-013)**: `BusinessSummaryService` — AI **chỉ thấy** `businessContextPromptText(BusinessContext)` (test chứng minh boundary), BYOK/Local theo preference chain, twin rule-based khi AI off/offline, business rỗng KHÔNG tốn provider call; card on-demand trên Reports + provenance chip (provider vs Rule-based). Read-only: không mutate/workflow/action · **WTM-135 G-3B AI Recommendation**: `BusinessAiEngine` (runner chung ADR-TON-013: load context → guard → serialize → provider chain → rule fallback; Summary delegate) + `BusinessRecommendationService` — gợi ý hành động CHỈ để chủ shop tự quyết (không mutate/execute/side-effect), twin `ruleBasedBusinessRecommendations` từ tín hiệu snapshot; nút "Gợi ý hành động" cùng card AI trên Reports |
